...
resourcename="payroll"  # lower case, only
resourcegroup="payroll_rg"
...
# git clone https://github.com/apilogicserver/payroll.git
# cd payroll
...
az group create --name payroll_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group payroll_rg --sku S1 --is-linux
az container create --resource-group payroll_rg --name payroll --image apilogicserver/payroll:latest --dns-name-label payroll --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//payroll.westus.azurecontainer.io:5656'
